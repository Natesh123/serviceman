...
class  DashboardController extends GetxController implements GetxService{
  ...
  Future<void> getDashboardData({bool reload = true}) async {
    try {
      ...
      if (kDebugMode) {
        print("Dashboard API Status: ${response.statusCode}, Body: ${response.body}");
      }
      ...
      if(response.statusCode == 200){
        ...
        if (content is List) {
          for (var item in content) {
            if (kDebugMode) print("Processing Item: $item"); 
            if (item is Map && item.containsKey('top_cards')) {
              if (kDebugMode) print("Found Top Cards");
              _topCards = TopCards.fromJson(item['top_cards']);
            }
            if (item is Map && item.containsKey('bookings')) {
              if (kDebugMode) print("Found Bookings");
              _bookings = item['bookings'];
            }
          }
        }
