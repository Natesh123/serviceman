...
class ConversationController extends GetxController  with GetSingleTickerProviderStateMixin implements GetxService{
  ...
  void onInit(){
    ...
    channelScrollController2.addListener(() {
      if (channelScrollController2.position.pixels == channelScrollController2.position.maxScrollExtent) {
  ...
  Future<void> getUnreadConversationCount() async {
    Response response = await conversationRepo.getUnreadConversationCount();
    if (response.statusCode == 200) {
      _unreadConversationCount = response.body['content']['unread_conversation'] ?? 0;
    }
    update();
  }

  int getUnreadCustomerCount() {
    int count = 0;
    if (_customerChannelList != null) {
      for (var channel in _customerChannelList!) {
        int? isRead;
        if (channel.channelUsers != null && channel.channelUsers!.length > 1) {
          isRead = channel.channelUsers![0].user?.userType == "provider-serviceman" ? channel.channelUsers![0].isRead! : channel.channelUsers![1].isRead!;
        }
        if (isRead == 0) {
          count++;
        }
  ...
  int getUnreadProviderCount() {
    int count = 0;
    if (_providerChannelList != null) {
      for (var channel in _providerChannelList!) {
        int? isRead;
        if (channel.channelUsers != null && channel.channelUsers!.length > 1) {
          isRead = channel.channelUsers![0].user?.userType == "provider-serviceman" ? channel.channelUsers![0].isRead! : channel.channelUsers![1].isRead!;
        }
