...
class BookingDetailsRepo{
  ...
  Future<Response> changeBookingStatus({required String bookingID,required String status, required  String otp, List<MultipartBody>? photoEvidence, required bool isSubBooking, String? bookingNote}) async {
    return await apiClient.postMultipartData("${ isSubBooking ? AppConstants.subBookingStatusUpdateUrl : AppConstants.bookingStatusUpdateUrl}/$bookingID",
        {'booking_status':status,'_method':'put', "booking_otp": otp, 'booking_note': bookingNote ?? ''}, photoEvidence,null
    );
  }
