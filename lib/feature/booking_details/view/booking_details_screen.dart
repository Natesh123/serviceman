      ...
      builder: (bookingDetailsController){
        return CustomPopScopeWidget(
          child: Scaffold(
            ...
            floatingActionButton:  bookingDetailsController.isShowChattingButton(bookingDetailsController.bookingDetails?.bookingContent?.bookingDetailsContent) ? Padding(
              padding: const EdgeInsets.only(bottom: 50),
              child: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  if(bookingDetailsController.bookingDetails?.bookingContent?.bookingDetailsContent?.provider?.companyPhone != null || bookingDetailsController.bookingDetails?.bookingContent?.bookingDetailsContent?.provider?.contactPersonPhone != null)
                    SizedBox(
                      height: 50, width: 50,
                      child: FloatingActionButton(
                        ...
                        onPressed: () {
                          String phone = bookingDetailsController.bookingDetails?.bookingContent?.bookingDetailsContent?.provider?.companyPhone ?? bookingDetailsController.bookingDetails?.bookingContent?.bookingDetailsContent?.provider?.contactPersonPhone ?? "";
                          String finalPhone = ValidationHelper.getValidPhone(phone, withCountryCode: false);
                          if (finalPhone.isEmpty) {
                            finalPhone = phone.replaceAll(RegExp(r'[^\d]'), '');
                            if(finalPhone.length > 10) {
                              finalPhone = finalPhone.substring(finalPhone.length - 10);
                            }
                          }
                          if(finalPhone.isNotEmpty) {
                            launchUrl(Uri.parse('tel:$finalPhone'), mode: LaunchMode.externalApplication);
                          }
                        },
                        child: Icon(Icons.call, color: light.cardColor,),
                      ),
                    ),
