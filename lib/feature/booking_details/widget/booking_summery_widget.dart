...
class BookingSummeryView extends StatelessWidget{
  ...
  Widget build(BuildContext context){
    return GetBuilder<BookingDetailsController>(builder:(bookingDetailsController){
      ...
      double basePrice = bookingDetails.details?.first.service?.minBiddingPrice?.toDouble() ?? 0;
      double addonsTotal = 0;
      if (bookingDetails.details != null) {
        for (var detail in bookingDetails.details!) {
          if (detail.variantKey != null && detail.variantKey != "") {
            addonsTotal += ((detail.serviceCost?.toDouble() ?? 0) * (detail.quantity?.toInt() ?? 0));
          }
        }
      }
      
      double syncSubTotal = basePrice + addonsTotal;

      double syncGrandTotal = syncSubTotal;
      syncGrandTotal -= (bookingDetails.totalDiscountAmount ?? 0);
      syncGrandTotal -= double.tryParse(bookingDetails.totalCouponDiscountAmount ?? "0") ?? 0;
      syncGrandTotal -= double.tryParse(bookingDetails.totalCampaignDiscountAmount ?? "0") ?? 0;
      syncGrandTotal -= (bookingDetails.totalReferralDiscountAmount ?? 0);
      syncGrandTotal += (bookingDetails.extraFee ?? 0);
      ...
      if(isPartialPayment) {
        ...
        paidAmount  = syncGrandTotal - (bookingDetails.additionalCharge ?? 0);
      }

      double additionalCharge = isPartialPayment ? syncGrandTotal - paidAmount : bookingDetails.additionalCharge ?? 0;
      double dueAmount = syncGrandTotal - paidAmount;
              ...
              child: Column(children: [
                ...
                Padding(
                  ...
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      Text(bookingDetails.details?.first.serviceName ?? "Service", style: robotoRegular.copyWith(fontSize: Dimensions.fontSizeDefault)),
                      Text(PriceConverter.convertPrice(basePrice, isShowLongPrice: true), style: robotoRegular.copyWith(fontSize: Dimensions.fontSizeDefault)),
                    ],
                  ),
                ),
                ...
                ListView.builder(
                  ...
                  itemBuilder: (context,index){
                    // Only show addons in the list (Base service shown above)
                    if (bookingDetails.details?[index].variantKey == null || bookingDetails.details?[index].variantKey == "") {
                ...
                Row(
                  children: [
                    ...
                    Text(PriceConverter.convertPrice(syncSubTotal, isShowLongPrice:true),
                ...
                Row(
                  ...
                  children: [
                    ...
                    const SizedBox(width: Dimensions.paddingSizeDefault,),
                    Text("(-) ${PriceConverter.convertPrice(bookingDetails.totalDiscountAmount ?? 0, isShowLongPrice:true)}",
                      style: robotoRegular.copyWith(fontSize: Dimensions.fontSizeDefault,
                          color: Theme.of(context).textTheme.bodyLarge!.color?.withValues(alpha:0.9)
                      ),
                    ),
                ...
                Row(
                  ...
                  children: [
                    ...
                    Text("(-) ${PriceConverter.convertPrice(
                        double.tryParse(bookingDetails.totalCouponDiscountAmount ?? "0") ?? 0,
                ...
                Row(
                  ...
                  children: [
                    ...
                    const SizedBox(width: Dimensions.paddingSizeDefault,),
                    Text("(+) ${PriceConverter.convertPrice( 0, // Tax is 0 in SafeSync
                        isShowLongPrice:true)}",
                      style: robotoRegular.copyWith(fontSize: Dimensions.fontSizeDefault,
                          color: Theme.of(context).textTheme.bodyLarge!.color?.withValues(alpha:0.9)
                      ),
                ...
                Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  ...
                  Directionality(
                    textDirection: TextDirection.ltr,
                    child: Text(
                      PriceConverter.convertPrice(syncGrandTotal, isShowLongPrice: true),
